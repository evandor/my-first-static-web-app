...
function createPages(pageContentItems, pageLayout) {
    page_count++;
    ...
    const newPage = addPage(createPageFromLayout(pageLayout));
    ...
    while (pageContentItems.length > 0) {
        ...
        if (pageContentItem.classList.contains(pageBreakBeforeTag)) {
            ...
            createPages(pageContentItems, pageLayout);
        } else {
            ...
            if (isPageContentOverflowing(pageContent)) {
                ...
                // if there is a manual page break class attached to the item
                if (pageContentItem.classList.contains(pageBreakAfterTag)) {
        ...
        document.querySelectorAll(".a4")
            .forEach(
                page => {
                    ...
                    createPages(input.pageContentItems, input.pageLayout);
                }
            );
    } catch (e) {
